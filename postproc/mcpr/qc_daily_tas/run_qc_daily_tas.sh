...
if [ -e ~/RUCORE/${METHOD^^}/${RCP^^}/TASMAX_DAILY/ADJ/${METHOD}_county_daily_001_tasmax_${MOD}_${RCP}_19810101-22001231.nc ]; then continue; fi

echo "Working with [ RCP: $RCP ] [ Model: $MOD ] [Variables: tasmin, tas, tasmax]..."

fil1=`ls $root/${METHOD^^}/${RCP^^}/TAS_DAILY/${METHOD}_county_daily_${SET}_tas_${MOD}*.nc 2>/dev/null`
if [ ${#fil1[@]} -eq 0 ]; then continue; fi
fil2=`ls $root/${METHOD^^}/${RCP^^}/TASMIN_DAILY/${METHOD}_county_daily_${SET}_tasmin_${MOD}*.nc 2>/dev/null`
if [ ${#fil2[@]} -eq 0 ]; then continue; fi
fil3=`ls $root/${METHOD^^}/${RCP^^}/TASMAX_DAILY/${METHOD}_county_daily_${SET}_tasmax_${MOD}*.nc 2>/dev/null`
...
external QAQC "f90/qa_tmax_tmin.so"
 ...
 ncFil = "mcpr_county_daily_${SET}_tasmax_${MOD}_${RCP}_"+yr1+"0101-"+yr2+"1231_tmp.nc"
 ncFile1 = ncDir + "/" +ncFil
 ...
 ncFil = "mcpr_county_daily_${SET}_tasmax_${MOD}_${RCP}_"+yr1+"0101-"+yr2+"1231.nc"
 ncFile2 = ncDir + "/" +ncFil
 ...
 system("if ! test -d " + ncDir +" ; then mkdir -p " + ncDir + " ; fi")
 ncFil = "mcpr_county_daily_${SET}_tasmin_${MOD}_${RCP}_"+yr1+"0101-"+yr2+"1231_tmp.nc"
 ncFile1 = ncDir + "/" +ncFil
 ...
 ; replace original file
 ncFil = "mcpr_county_daily_${SET}_tasmin_${MOD}_${RCP}_"+yr1+"0101-"+yr2+"1231.nc"
 ncFile2 = ncDir + "/" +ncFil
 system("mv " + ncFile1 + " " +ncFile2)
