...
cat > cnty_exceed_mod2txt.ncl.tmp << EOF
...
; 2. Writes to a text file
 ...
 print("Mapping GHCND stations to county lat/lon centroids...")
 print("")
 ...
 do i=0, ncnty-1
  if ( cnty_lon(i)+360 .gt. 235 ) then
    ...
    indx = minind(dist)
    print("Mapping GHCND site: "+stn_code(indx)+ \
...
rm ncl.log
$ncl cnty_exceed_mod2txt.ncl.tmp | tee ncl.log
rm cnty_exceed_mod2txt.ncl.tmp
