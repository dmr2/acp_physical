...
for VAR in "tasmin" "tasmax"; do

fils=`ls $root/${METHOD}/${SET}/${RCP}/*/${VAR}/smme_ghcnd_mon*195001-*.nc 2>/dev/null`
...
cat > dwnscl_stn2stn.ncl.tmp << EOF
; dwnscl_stn2stn.ncl
...
; GHCND stations. The downscaling is at the GHCND station level.

; For each climate month in the CMIP5 forecast period 
; one year from the obs. climatology is randomly 
; selected. For each GHCND site, the obs. daily values
...
do iset = 1, $NSET
   ...
   print("Opening monthly average GCM projections (anomalies) at GHCND stations: ${FIL}")
   ...
   if "${VAR}" .ne. "pr" then
    xarr@units = "K"
    xarr@long_name = "Daily temperature projections at GHCND stations"
   else
    xarr@units = "mm"
    xarr@long_name = "Daily precip projections at GHCND stations"
   end if
   xarr&time = xtime
...
$ncl dwnscl_stn2stn.ncl.tmp | tee ncl.log
rm dwnscl_stn2stn.ncl.tmp
